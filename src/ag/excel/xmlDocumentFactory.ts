import {HeaderElement, PrefixedXmlAttributes, XmlElement} from '../types.ts';

const LINE_SEPARATOR = '\r\n';

export const xmlDocumentFactory = (() => {
    const createHeader = (headerElement: HeaderElement = {}): string => {
        const headerStart = '<?';
        const headerEnd = '?>';
        const keys = ['version'];
    ...
    const createXml = (xmlElement: XmlElement, booleanTransformer?:(currentValue:boolean) => string): string => {
        let props: string = '';
    ...
    const returnAttributeIfPopulated = (key: string, value: any, booleanTransformer?:(currentValue:boolean) => string) => {
